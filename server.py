...
マルチブラウザ対応:
  - 複数のブラウザ拡張機能（Chrome, Edge等）が同時に接続可能。
  - 各拡張機能は接続直後にidentifyメッセージを送信する:
      {"type": "identify", "browser": "chrome"}  または  {"browser": "edge"}
...
class ChromeKontrolServer:
    ...
    ブラウザ名（例: "chrome", "edge"）ごとに1つの接続済み拡張機能クライアントを管理する。
    ...
    async def send_command(
        ...
        Args:
            ...
            browser: 対象ブラウザ名（例: "chrome", "edge"）。Noneの場合:
    ...
    async def _resolve_client(
        ...
        if not self._clients:
            ...
            try:
                ...
                return {
                    'result': 'error',
                    'message': 'Timed out waiting for extension. Is ChromeKontrol loaded in Chrome or Edge?',
                }
...
async def run_serve_mode(ws_port: int, http_port: int) -> None:
    ...
    この関数の動作:
      1. Chrome拡張機能を受け入れるためにWebSocketサーバー（ポートws_port）をバインドする。
    ...
    env_var_name: str = 'CHROME_KONTROL_TOKEN'
...
def main() -> None:
    ...
    使用方法（サーブモード）:
        ...
        # 環境変数設定時: stderrにトークン値は表示されない（$CHROME_KONTROL_TOKEN をそのまま使用）
        export CHROME_KONTROL_TOKEN=your_fixed_token_here  # 任意
        python3 server.py --serve
        curl -s localhost:9766 -H "X-ChromeKontrol-Token: $CHROME_KONTROL_TOKEN" \
