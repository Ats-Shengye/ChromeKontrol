...
| Service Worker | MV3 拡張機能のバックグラウンドスクリプト。イベント駆動で DOM アクセス不可、メモリ上の永続状態なし。`background.js` がこれに該当（Chrome/Edge）。Firefox では代わりに event page が使われる |
| Event page | Firefox の MV3 実装が使う非永続バックグラウンド実行モデル。`manifest.json` の `background.scripts`（`service_worker` ではなく）で指定する。Chrome/Edge の Service Worker と役割は同じだが起動・停止の実装が異なる |
| コンテンツスクリプト | 拡張機能から Web ページに注入されるスクリプト。ページ JS とは隔離された Isolated World で動作。ChromeKontrol の `content.js` は意図的に空 |
| `browser_specific_settings` | Firefox 固有の `manifest.json` トップレベルフィールド。`gecko.id`（拡張の一意識別子、`{UUID}` 形式）と `gecko.strict_min_version`（対応する最小 Firefox バージョン）を指定する。Chrome/Edge の manifest には存在しない |
| 一時的なアドオン（temporary add-on） | Firefox で署名なしの拡張機能を読み込む方式。`about:debugging#/runtime/this-firefox` の「一時的なアドオンを読み込む」で `manifest.json` ファイルを直接選択する（Chrome/Edge の「ディレクトリを選択」とは異なる）。ブラウザを再起動すると読み込みが解除される |

## Chrome Extension API

Phase F3a 以降、`background.js` 実装コード上の実際の参照は `api.*`
（実行環境に `browser` があればそちら、Chrome/Edge では `chrome`）に
統一されている。以下の表は API 自体の機能説明であり、API 名は
WebExtension 標準の呼称（`chrome.*`）で記載する。
...
| マルチブラウザルーティング | 単一サーバーから Chrome / Edge / Firefox を同時制御。コマンドの `"browser"` フィールドで対象指定。1接続時は自動選択 |
...
| `extensions/chromium/background.js` | MV3 Service Worker。WebSocket 中継 + コマンドルーティング + DOM操作注入 |
| `extensions/chromium/content.js` | コンテンツスクリプト（意図的に空。DOM操作は `executeScript` で注入） |
| `extensions/chromium/manifest.json` | 拡張機能マニフェスト（MV3、Chrome/Edge用） |
| `extensions/firefox/background.js` | `extensions/chromium/background.js` とバイト単位で同一 |
| `extensions/firefox/content.js` | `extensions/chromium/content.js` とバイト単位で同一 |
| `extensions/firefox/manifest.json` | 拡張機能マニフェスト（MV3、Firefox用。`permissions` / `background` / CSP / `browser_specific_settings` が Chrome/Edge 版と異なる） |
| `extensions/tests/parity.test.mjs` | `chromium/` と `firefox/` のコード同一性を検証するテスト |
...
| ブラウザ名制限 | `chrome` / `edge` / `firefox` のホワイトリストで制限 |
